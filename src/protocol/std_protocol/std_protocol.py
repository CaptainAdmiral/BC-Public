...
import random
import time
from typing import Any, Awaitable, Callable, Self, cast
from network_emulator import NetConnection, network as net
from protocol import AbstractProtocol
from protocol.credit.receipt.receipt import Receipt
...
class StdProtocol(AbstractProtocol):
    ...
    def __init__(self, node):
        ...
        '''Dictionary of receipts keyed by which address currently holds the credit'''
    ...
    async def write_out_broadcast(self, data: BroadcastData):
        ...
        for node_data in selected_nodes:
            ...
            node = net.get_node(node_data.address)
            nc = await net.connect_to_node(self._node, node)
            with nc:
