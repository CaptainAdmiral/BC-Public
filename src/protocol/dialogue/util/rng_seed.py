...
class TransactionRNGSeed(RNGSeed):
    ...
    def __post_init__(self):
        if not 0 <= self.mod_payee_public_key < RNG_MOD_PK:
            raise ValueError(
                f"mod_payee_public_key must be less than {RNG_MOD_PK}, got {self.mod_payer_public_key}"
            )
        if not 0 <= self.mod_payer_public_key < RNG_MOD_PK:
            raise ValueError(
                f"mod_payer_public_key must be less than {RNG_MOD_PK}, got {self.mod_payee_public_key}"
            )
    ...
    def seed_bytes(self) -> bytes:
        ...
        # the same checksum.
        return to_bytes(
            (self.checksum, self.mod_payee_public_key, self.mod_payer_public_key)
        )
...
class ClaimStakeRNGSeed(RNGSeed):
    ...
    def __post_init__(self):
        if not 0 <= self.mod_claimant_pk < RNG_MOD_PK:
            raise ValueError(
                f"mod_payee_public_key must be less than {RNG_MOD_PK}, got {self.mod_claimant_pk}"
            )

    def seed_bytes(self) -> bytes:
        return to_bytes((self.checksum, self.mod_claimant_pk))


def get_transaction_rng(
    ...
    return TransactionRNGSeed(
        RNGSeedType.TRANSACTION,
        checksum=checksum,
        mod_payer_public_key=mod_payer_pk,
        mod_payee_public_key=mod_payee_pk,
    )


def get_claim_stake_rng(
    vnt: "VerificationNetTimeline", timestamp: float, claimant_public_key: str
):
    ...
    checksum = vnt.get_latest_checksum(timestamp - TIME_TO_CONSISTENCY)
    mod_claimant_pk = hash(claimant_public_key) % RNG_MOD_PK
    return ClaimStakeRNGSeed(
        RNGSeedType.CLAIM_STAKE, checksum=checksum, mod_claimant_pk=mod_claimant_pk
    )
...
RNGSeedTypes = (
    Annotated[TransactionRNGSeed, Tag(RNGSeedType.TRANSACTION)]
    | Annotated[ClaimStakeRNGSeed, Tag(RNGSeedType.CLAIM_STAKE)]
)
