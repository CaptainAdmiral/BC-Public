...
from settings import SIG_KEY_SIZE, SIG_PUBLIC_EXPONENT, SIG_HASH, SIG_PADDING
...
def dataclass_to_bytes(dataclass: object) -> bytes:
    if not is_dataclass(dataclass):
        raise TypeError("Expected a dataclass instance")
    return json.dumps(asdict(dataclass), sort_keys=True, separators=(",", ":")).encode("utf-8") # type: ignore

def to_bytes(obj: Any) -> bytes:
    return json.dumps(obj, separators=(",", ":")).encode("utf-8")

def to_sha256(obj: Any):
    return hashlib.sha256(to_bytes(obj)).hexdigest()
...
class RandomGen:
    ...
    def next(self, max_value: int):
        ...
        while True:
            hash_bytes = hashlib.sha256(self.seed_bytes + self.count.to_bytes(8, 'big')).digest()
